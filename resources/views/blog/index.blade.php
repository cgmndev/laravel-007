<x-app-layout>
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('blog.store') }}">
            @csrf
            <textarea name="message" placeholder="{{ __('Ingrese mensaje') }}"
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            @if ($post->user->is(auth()->user()))
                                <x-dropdown>
                                    ...
                                    <x-slot name="content">
                                        <x-dropdown-link :href="route('blog.edit', $post)">
                                            {{ __('Editar') }}
                                        </x-dropdown-link>
                                        <form method="POST" action="{{ route('blog.destroy', $post) }}">
                                            @csrf
                                            @method('delete')
                                            <x-dropdown-link :href="route('blog.destroy', $post)" onclick="event.preventDefault(); this.closest('form').submit();">
                                                {{ __('Eliminar') }}
                                            </x-dropdown-link>
                                        </form>
                                    </x-slot>
